...
export async function POST(request: Request) {
  try {
    ...
    const submissionPayload = {
      ...
      result_json:
        existingResultJson.journeyState !== undefined
          ? {
              ...result,
              journeyState: existingResultJson.journeyState,
            }
          : result,
    }
    ...
    if (latestSubmission?.id) {
      ...
      if (updateError) {
        ...
        return NextResponse.json({ success: false, error: 'Failed to update readiness profile' }, { status: 500 })
      }
    ...
    if (insertError) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to save readiness profile' }, { status: 500 })
    }
